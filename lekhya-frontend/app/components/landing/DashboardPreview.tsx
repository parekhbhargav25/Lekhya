...
export default function DashboardPreview() {
  ...
  return (
    <motion.div
      ...
      className="w-full rounded-3xl bg-white/80 border border-slate-200 shadow-xl backdrop-blur-md p-5 md:p-6"
    >
        ...
        {sample.map((r) => (
          ...
          >
            <div>
              ...
              <div className="text-[11px] text-slate-500">Auto • {r.tag}</div>
            </div>
            <span className="text-xs font-semibold text-slate-900">{r.amount}</span>
          </div>
        ))}
      </div>
    </motion.div>
