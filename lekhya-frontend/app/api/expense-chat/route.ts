...
//   // Clean merchant phrase (remove punctuation)
//   const merchantQuery = afterAt.replace(/[.?]/g, "").trim(); // e.g. "costco", "costco wholesale"
//   if (!merchantQuery) return null;
...
//   const entries = Object.entries(totalsByItem).filter(([, v]) => v.total > 0);
...
//       userId,
//       status: "parsed",
//       // we assume parsed receipts have extractedJson set; no Prisma.JsonNull needed
//     },
//     orderBy: { createdAt: "asc" },
//   });
...
//       merchant: parsed?.merchant ?? "Unknown merchant",
//       category:
//         r.categoryOverride ?? parsed?.category ?? "Uncategorized",
//       total: parsed?.total ?? 0,
//       tax: parsed?.tax ?? null,
//       paymentMethod: parsed?.paymentMethod ?? null,
//       lineItems,
...
//   // 5) First try our custom item-level logic
...
//   // 6) LangChain: prompt + model + simple chain
//   const prompt = PromptTemplate.fromTemplate(`
// You are Lekhya, a friendly personal expense assistant.
...
// Each item includes: id, date, merchant, category, total, tax, paymentMethod, and lineItems.
...
export const runtime = "nodejs";          // ensure Node runtime for LangChain
...
export async function POST(req: NextRequest) {
  // 1) Auth
  const session = await getServerSession(authOptions);
  const userId = session?.user?.email; // we’re using email as userId
  ...
  // 3) Fetch this user’s parsed receipts
  const receipts = await prisma.receipt.findMany({
    where: {
      userId,
      status: "parsed",
      // assuming parsed receipts have extractedJson filled
    },
    orderBy: { createdAt: "asc" },
  });
  ...
  // 5) First: custom rule for "max item at merchant"
  const directItemAnswer = tryAnswerMaxItemAtMerchant(
    question,
    expenseDocs
  );
  ...
  // 6) Build JSON for the LLM
  const expensesJson = JSON.stringify(expenseDocs, null, 2);

  // 7) LangChain prompt + model
  const prompt = PromptTemplate.fromTemplate(`
You are Lekhya, a friendly personal expense assistant.
...
`);
  ...
  try {
    ...
    console.error("LangChain expense-chat error:", err);
    return NextResponse.json(
      {
        answer:
          "Sorry, something went wrong while analyzing your expenses. Please try again.",
      },
      { status: 500 }
    );
  }
