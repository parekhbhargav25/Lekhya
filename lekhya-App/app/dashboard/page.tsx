...
export default function OverviewPage() {
  const { receipts, isInRange, getCategory, duplicates } = useDashboard();
    ...
    <>
      {duplicates.length > 0 && (
        <Link
          ...
          className="mb-5 flex items-center justify-between gap-4 rounded-2xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 px-5 py-3 shadow-sm hover:shadow-md transition-shadow"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">
                {duplicates.length} possible duplicate {duplicates.length === 1 ? "charge" : "charges"} detected
              </p>
              <p className="text-xs text-slate-600">
                Up to ${potentialSavings.toFixed(2)} in potentially duplicated spend · review now
              </p>
            </div>
          </div>
